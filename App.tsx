...
const App: React.FC = () => {
  ...
  // Kiểm tra nếu chưa có Key thì hiện Modal sau 1.5 giây
  useEffect(() => {
    ...
    <div className="min-h-screen bg-background flex flex-col relative">
      ...
      <header className="bg-primary text-white sticky top-0 z-50 shadow-md px-4">
        <div className="max-w-7xl mx-auto h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => setActiveTab(allTabs[0])}>
            ...
            <span className="text-[8px] font-black uppercase tracking-widest opacity-90 leading-tight">Tiêu Quang Thạch</span>
          </div>
          <div className="hidden md:block flex-grow h-7 bg-white/10 rounded-full flex items-center overflow-hidden text-center">
            <span className="w-full text-sm font-bold uppercase italic animate-marquee whitespace-nowrap px-4">
              Hệ thống dạy học và kiểm tra tạo bởi AI
            </span>
          </div>
          <button className="bg-secondary px-5 py-2 rounded-lg text-sm font-bold shadow-sm hover:brightness-110 transition active:scale-95">Đăng ký</button>
        </div>
      </header>
          ...
          <ul className="flex justify-center items-center gap-6 py-3 text-[13px] font-black uppercase tracking-tight">
            ...
            <li className="relative" ref={dropdownRef}>
              <button 
                ...
                className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/20"
              >
      ...
      {showApiModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fade-in">
          <div className="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl relative overflow-hidden border border-gray-100">
            <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-primary to-secondary"></div>
            <h3 className="text-2xl font-black text-primary mb-2 italic uppercase">Cấu hình API Key</h3>
            <p className="text-gray-400 text-sm mb-8 leading-relaxed">Dán API Key của bạn để bắt đầu tạo đề kiểm tra bằng trí tuệ nhân tạo.</p>
            <input 
              type="password" 
              value={tempKey} 
              onChange={(e) => setTempKey(e.target.value)} 
              placeholder="Dán mã Gemini API Key tại đây..." 
              className="w-full px-6 py-4 rounded-2xl border-2 border-gray-100 focus:border-primary outline-none mb-6 font-mono text-sm bg-gray-50 shadow-inner" 
            />
            ...
            <p className="mt-8 text-center text-[11px] font-bold text-gray-300">LẤY MÃ MIỄN PHÍ TẠI <a href="https://aistudio.google.com/app/apikey" target="_blank" className="text-secondary underline underline-offset-4">GOOGLE AI STUDIO →</a></p>
          </div>
        </div>
      )}
      ...
      <button 
        ...
        className="fixed bottom-8 right-8 bg-primary text-white p-5 rounded-full shadow-2xl z-50 hover:scale-110 active:scale-90 transition-all border-4 border-white shadow-primary/40"
      >
